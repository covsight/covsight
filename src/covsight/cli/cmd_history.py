...
def _ts(ts_arg: str) -> int | None:
    ...
    try:
        ...
        import datetime
        return int(datetime.datetime.fromisoformat(ts_arg).timestamp())
...
def cmd_history_query(args) -> None:
    ...
    try:
        if fmt == "json":
            ...
            out.write(f"{'Date':<20} {'Status':<8} {'Seed':>12}\n")
            out.write("-" * 42 + "\n")
            for r in records:
                date = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(r.ts))
                status = "pass" if r.status == 0 else "fail"
...
def register(subparsers):
    parser = subparsers.add_parser("history", help="Query NCDB test history")
    history_sub = parser.add_subparsers(dest="history_cmd")

    query = history_sub.add_parser("query", help="Display history records for a specific test")
    query.add_argument("db", help="Path to the NCDB .cdb file")
    query.add_argument("test_name", help="Test name to query")
    query.add_argument("--from", dest="from_", metavar="DATE", default=None, help="Start date (ISO 8601 or Unix timestamp)")
    query.add_argument("--to", metavar="DATE", default=None, help="End date (ISO 8601 or Unix timestamp)")
    query.add_argument("--out", "-o", default=None, help="Output file")
    query.add_argument("--output-format", "-of", default="text", choices=["text", "json"], help="Output format")
    query.set_defaults(func=cmd_history_query)

    stats = history_sub.add_parser("stats", help="Show flaky/failing test statistics")
    stats.add_argument("db", help="Path to the NCDB .cdb file")
    stats.add_argument("test_name", nargs="?", default=None, help="Show stats for a specific test name")
    stats.add_argument("--top-flaky", metavar="N", type=int, default=None, help="Show top N flaky tests")
    stats.add_argument("--top-failing", metavar="N", type=int, default=None, help="Show top N failing tests")
    stats.add_argument("--out", "-o", default=None, help="Output file")
    stats.add_argument("--output-format", "-of", default="text", choices=["text", "json"], help="Output format")
    stats.set_defaults(func=cmd_history_stats)
